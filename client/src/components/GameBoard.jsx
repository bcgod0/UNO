...
export default function GameBoard({
  ...
  return (
    <div className="relative min-h-screen w-full bg-slate-950 flex flex-col justify-between overflow-hidden select-none mobile-landscape-board">
      ...
      {/* Top Header Bar */}
      <header className="relative z-20 flex items-center justify-between px-2 sm:px-4 py-1.5 sm:py-3 bg-slate-900/90 border-b border-slate-800 backdrop-blur-md mobile-landscape-header">
        <div className="flex items-center gap-1.5 sm:gap-3">
          <span className="font-title font-black text-lg sm:text-2xl text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-yellow-400 to-green-400 animate-float-slow">
            UNO!
          </span>
          <div className="px-1.5 py-0.5 sm:px-3 sm:py-1 bg-slate-800 border border-slate-700 rounded-lg text-[9px] sm:text-xs font-mono font-bold text-yellow-400">
            ROOM: {code}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 sm:px-2.5 sm:py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-semibold text-slate-300 transition-all"
          ...
          <button
            onClick={onLeaveRoom}
            className="flex items-center gap-1 px-2 py-1 sm:px-2.5 sm:py-1.5 bg-red-950/70 hover:bg-red-900/90 border border-red-800/80 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-semibold text-red-300 hover:text-white transition-all cursor-pointer"
            title="Leave Game"
          >
      ...
      {showLogs && (
        <div className="fixed top-12 sm:top-16 right-2 sm:right-4 z-40 w-64 sm:w-80 max-h-60 sm:max-h-80 bg-slate-900/95 border border-slate-800 rounded-xl sm:rounded-2xl p-2.5 sm:p-4 shadow-2xl backdrop-blur-lg flex flex-col">
          <h3 className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
            <ScrollText className="w-3.5 h-3.5 text-indigo-400" />
            <span>Match Event Feed</span>
          </h3>
          <div className="flex-1 overflow-y-auto space-y-1 text-[11px] sm:text-xs font-medium text-slate-300 pr-1">
            {logs && logs.length > 0 ? (
              logs.map((log, idx) => (
                <div key={idx} className="p-1.5 bg-slate-950/60 rounded-lg border border-slate-800/60">
                  {log}
                </div>
              ))
            ) : (
      ...
      {/* Error Banner */}
      {activeError && (
        <div className="absolute top-12 sm:top-16 left-1/2 -translate-x-1/2 z-30 px-3 py-1 sm:px-6 sm:py-2 bg-red-500/90 text-white font-bold text-[11px] sm:text-sm rounded-full shadow-lg border border-red-400 animate-bounce text-center max-w-[90vw]">
          ⚠️ {activeError}
        </div>
      )}

      {/* Center Table Area */}
      <main className="relative flex-1 flex flex-col items-center justify-between p-1.5 sm:p-4 max-w-6xl w-full mx-auto">
        {/* Opponents Section */}
        <div className="w-full flex flex-wrap justify-center items-center gap-1.5 sm:gap-4 pt-1 mb-1 sm:mb-4 max-h-32 sm:max-h-none overflow-y-auto mobile-landscape-opponents">
          {otherPlayers.map((opp) => (
            <div
              key={opp.id}
              className={`relative flex flex-col items-center p-1.5 sm:p-3 rounded-lg sm:rounded-2xl transition-all duration-300 ${
                opp.isCurrentTurn
            ...
            >
              ...
              {unoToast && unoToast.playerId === opp.id && (
                <div className="absolute -top-2.5 -right-1.5 z-30 px-2 py-0.5 sm:px-3 sm:py-1 bg-gradient-to-r from-red-600 via-yellow-500 to-red-600 text-white font-black text-[9px] sm:text-xs rounded-full shadow-xl border border-yellow-300 animate-bounce flex items-center gap-0.5 backdrop-blur-md">
                  <Flame className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-yellow-300 fill-current animate-pulse" />
                  <span>📢 UNO!</span>
                </div>
              )}
              ...
              <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-2">
                <div className="w-5 h-5 sm:w-8 sm:h-8 rounded-md sm:rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-[10px] sm:text-sm text-white">
                  {opp.name.charAt(0).toUpperCase()}
                </div>
                <div className="text-left">
                  <div className="text-[10px] sm:text-xs font-bold text-white leading-tight max-w-[65px] sm:max-w-none truncate">{opp.name}</div>
                  <div className="text-[8px] sm:text-[10px] text-slate-400 font-semibold">{opp.cardCount} Cards</div>
                </div>
              </div>
              ...
              {opp.isCurrentTurn && (
                <div className={`mt-1 sm:mt-2 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full font-black text-[9px] sm:text-xs uppercase tracking-wider flex items-center gap-0.5 shadow-lg border ${
                  timeLeft <= 5
              ...
              {opp.cardCount === 1 && !opp.calledUno && (
                <button
                  ...
                  className="mt-1 sm:mt-2 px-2 py-0.5 sm:px-3 sm:py-1 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-extrabold text-[8px] sm:text-[10px] uppercase rounded-full shadow-lg border border-red-400 animate-pulse cursor-pointer"
                >
                  🚨 Catch UNO!
                </button>
              )}
              ...
              {opp.cardCount === 1 && opp.calledUno && (
                <div className="mt-1 sm:mt-2 px-2 py-0.5 sm:px-3 sm:py-1 bg-emerald-500 text-slate-950 font-black text-[8px] sm:text-[10px] uppercase rounded-full shadow border border-emerald-300 flex items-center gap-0.5">
        ...
        <div className="relative my-auto flex flex-col items-center gap-1.5 sm:gap-4 w-full">
          {/* STACKED DRAW PENALTY WARNING BANNER */}
          {pendingDrawCount > 0 && (
            <div className="px-2.5 py-0.5 sm:px-5 sm:py-1.5 bg-gradient-to-r from-red-600 via-rose-600 to-amber-500 text-white font-black text-[9px] sm:text-xs uppercase tracking-wider rounded-full shadow-lg border border-yellow-300 animate-pulse flex items-center gap-1 text-center max-w-[95vw]">
              <Zap className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-300 fill-current flex-shrink-0" />
              <span>
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-4 bg-slate-900/95 border border-slate-800 px-2.5 sm:px-6 py-1 sm:py-2.5 rounded-full shadow-2xl backdrop-blur-md max-w-full mobile-landscape-pill">
            {/* Color Badge */}
            <div className="flex items-center gap-1 sm:gap-2">
              <span className="text-[9px] sm:text-xs font-bold uppercase tracking-wider text-slate-400">COLOR:</span>
              <span className={`px-2.5 py-0.5 sm:px-4 sm:py-1 rounded-full font-black text-[9px] sm:text-xs uppercase shadow-md ${colorBadgeClasses}`}>
                {currentColor || 'ANY'}
              </span>
            </div>

            <div className="hidden xs:block h-3.5 sm:h-5 w-px bg-slate-700"></div>

            {/* LARGER PROMINENT DIRECTION BADGE */}
            <div className={`px-2.5 py-0.5 sm:px-4 sm:py-1 rounded-full font-black text-[9px] sm:text-sm uppercase tracking-wider flex items-center gap-1 sm:gap-2.5 shadow-lg border ${
              direction === 1
              ...
              {direction === 1 ? (
                <>
                  <div className="w-4 h-4 sm:w-7 sm:h-7 rounded-full bg-slate-950 flex items-center justify-center text-emerald-400 shadow-inner">
                    <RotateCw className="w-2.5 h-2.5 sm:w-4 sm:h-4 animate-spin-slow stroke-[3]" />
                  </div>
                  <span className="tracking-wide text-[9px] sm:text-sm">CLOCKWISE</span>
                  <ArrowRight className="w-3 h-3 sm:w-5 sm:h-5 stroke-[3] animate-pulse" />
                </>
              ) : (
                <>
                  <div className="w-4 h-4 sm:w-7 sm:h-7 rounded-full bg-slate-950 flex items-center justify-center text-amber-400 shadow-inner">
                    <RotateCcw className="w-2.5 h-2.5 sm:w-4 sm:h-4 animate-spin-reverse-slow stroke-[3]" />
                  </div>
                  <span className="tracking-wide text-[9px] sm:text-sm">ANTI-CLOCKWISE</span>
                  <ArrowLeft className="w-3 h-3 sm:w-5 sm:h-5 stroke-[3] animate-pulse" />
                </>
              )}
            </div>
          </div>
          ...
          {/* Cards Table Surface */}
          <div className="flex items-center justify-center gap-3 sm:gap-10 p-2 sm:p-6 bg-slate-900/50 border border-slate-800/80 rounded-xl sm:rounded-3xl backdrop-blur-sm shadow-2xl mobile-landscape-table-surface">
            ...
            <div className="flex flex-col items-center gap-0.5 sm:gap-2">
              ...
              <div className={`relative transition-transform ${isDrawing ? 'animate-draw-card-lift' : ''}`}>
                ...
                <button
                  ...
                  disabled={!isMyTurn}
                  className={`absolute inset-0 rounded-lg sm:rounded-2xl flex flex-col items-center justify-center font-black text-[9px] sm:text-xs text-white uppercase tracking-wider transition-all bg-black/30 backdrop-blur-[2px] ${
            ...
            <div className="flex flex-col items-center gap-0.5 sm:gap-2">
              <span className="text-[8px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-wider">Discard Pile</span>
              <div
              ...
              >
                {topDiscard ? (
                  ...
                  <div className="w-14 h-22 xs:w-16 xs:h-24 sm:w-28 sm:h-44 rounded-lg sm:rounded-2xl border-2 border-dashed border-slate-700 flex items-center justify-center text-slate-500 text-[10px] sm:text-xs font-bold card-large-landscape">
                    Empty
                  </div>
                )}
              </div>
        ...
        <div className="w-full flex flex-col items-center gap-1.5 sm:gap-3 pb-1 sm:pb-2 z-20">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 mobile-landscape-action-bar">
            {me.calledUno ? (
              <div className="px-4 py-1.5 sm:px-8 sm:py-3 bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-500 text-white font-black text-xs sm:text-lg tracking-wider rounded-lg sm:rounded-2xl shadow-xl shadow-emerald-950/60 border border-emerald-300 flex items-center gap-1 sm:gap-2 animate-pulse mobile-landscape-uno-btn">
                <CheckCircle2 className="w-3.5 h-3.5 sm:w-6 sm:h-6 text-yellow-300" />
                <span>UNO CALLED!</span>
              </div>
            ) : (
              <button
                onClick={handleCallUnoClick}
                className={`px-4 py-1.5 sm:px-8 sm:py-3 bg-gradient-to-r from-red-600 via-rose-600 to-yellow-500 hover:from-red-500 hover:to-yellow-400 text-white font-black text-xs sm:text-lg tracking-wider rounded-lg sm:rounded-2xl shadow-xl shadow-red-950/60 border border-yellow-300 flex items-center gap-1 sm:gap-2 transition-all transform active:scale-95 cursor-pointer mobile-landscape-uno-btn ${
                  me.hand.length <= 2 ? 'opacity-100' : 'opacity-70'
                }`}
              >
                <Flame className="w-3.5 h-3.5 sm:w-6 sm:h-6 text-yellow-300 fill-current" />
                <span>SHOUT UNO!</span>
              </button>
            )}
            ...
            {unoToast && unoToast.playerId === myId && (
              <div className="px-2 py-0.5 sm:px-3 sm:py-1.5 bg-gradient-to-r from-red-600 via-yellow-500 to-red-600 text-white font-black text-[9px] sm:text-xs rounded-full shadow-lg border border-yellow-300 animate-bounce flex items-center gap-0.5 sm:gap-1.5">
          ...
          <div className="w-full max-w-5xl px-1 sm:px-4 flex justify-center items-end -space-x-3 xs:-space-x-4 sm:-space-x-6 overflow-x-auto pb-1 sm:pb-4 pt-2 sm:pt-6 touch-pan-x scrollbar-none mobile-landscape-hand-container">
